...
/** Build ClayCode image with optional silhouette overlay. */
const buildClaycodeImage = async (downloadCode: string): Promise<Buffer> => {
  ...
  try {
    ...
    return await sharp(claycodeBuffer)
      .composite([{
...
export const handler = async (event: PrintPrepareInput): Promise<PrintPrepareOutput> => {
  ...
  if (downloadCode) {
    const claycodeImage = await buildClaycodeImage(downloadCode)
